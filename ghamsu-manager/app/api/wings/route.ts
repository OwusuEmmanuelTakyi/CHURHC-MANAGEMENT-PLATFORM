...
export async function GET(req: Request) {
  try {
    ...
    let q = db.from('wings').select('id, name, local_id').order('name');
    q = scopeQuery(ctx, q);
    if (ctx.role === 'national_president' && localId) q = q.eq('local_id', Number(localId));

    const { data, error } = await q;
    if (error) throw new ApiError(500, error.message);
    return NextResponse.json({ wings: data });
...
export async function POST(req: Request) {
  try {
    ...
    const { data, error } = await db.from('wings')
    ...
    await audit(ctx, 'wing.created', 'wing', data.id, { name: body.name });
